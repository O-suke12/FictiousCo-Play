...
class LILI:
    def __init__(
        ...
        **kwargs,
        ...
        self.cfg = cfg
        self.policy = ActorCritic(state_dim, action_dim, self.z_dim).to(device)
        ...
        self.policy_old = ActorCritic(state_dim, action_dim, self.z_dim).to(device)
    ...
    def update(self):
        ...
        for _ in range(self.cfg.K_epochs):
            ...
            latent_strategies = self.encoder(tau[:, :-1])
            state_latent = torch.cat((old_states[1:-1, :], latent_strategies), dim=1)

            logprobs, state_values, dist_entropy = self.policy.evaluate(
                state_latent, old_actions[1:-1]
            )
            ...
            ratios = torch.exp(logprobs - old_logprobs[1:-1].detach())

            # Finding Surrogate Loss
            surr1 = ratios * advantages[1:-1]
            surr2 = (
            ...
            critique_loss = self.MseLoss(state_values, rewards[1:-1])
            ...
            # encoder_loss = critique_loss.detach() + J_rep
