...
class Workspace:
    def __init__(self, cfg: DictConfig):
        ...
        self.memory = ReplayMemory(
            capacity=self.cfg.num_eps, interaction_length=self.cfg.max_episode_steps
        )
                    ...
                    if agent == self.env.agents[0]:
                        ...
                        if len(self.memory) > self.cfg.batch_size:
                            (
                                ...
                                self.memory, self.cfg.batch_size
                            )
                        ...
                        mask = (
                            1
                            if episode_steps == self.cfg.max_episode_steps
                            else float(not done)
                        )
                        self.memory.push_timestep(
