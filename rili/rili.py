...
from rili.model_rili import RILI_Autoencoder
from rili.model_sac import GaussianPolicy, QNetwork
from utils.utils import hard_update, soft_update


class RILI(object):
    def __init__(self, action_dim, state_dim, timestep):
        # hyperparameters
        self.gamma = 0.99
        self.tau = 0.005
        self.alpha = 0.2
        ...
        # Autoencoder
        self.autoencoder = RILI_Autoencoder(
            state_dim=state_dim,
            reward_dim=1,
            latent_dim=10,
            hidden_dim=64,
    ...
    def predict_latent(self, interaction1, interaction2, interaction3, interaction4):
        ...
        z, z_mean, z_std, p, p_mean, p_std = self.autoencoder.encoder(
            tau1, tau2, tau3, tau4
        )
        return torch.cat((z, z_std, p), -1).detach().cpu().numpy()

    def predict_latent_gaussian(
        self, interaction1, interaction2, interaction3, interaction4
    ):
        states1, _, rewards1, _, _ = map(np.stack, zip(*interaction1))
        states2, _, rewards2, _, _ = map(np.stack, zip(*interaction2))
        states3, _, rewards3, _, _ = map(np.stack, zip(*interaction3))
        ...
        return (
            torch.cat((z, z_std, p), -1).detach().cpu().numpy(),
    ...
    def update_parameters(self, memory, batch_size):
        ...
        ae_loss, curr_loss, next_loss, kl_loss = self.autoencoder(
            tau1,
            tau2,
            tau3,
            tau4,
        ...
        z_curr, _, z_curr_std, p_curr, _, _ = self.autoencoder.encoder(
            tau1, tau2, tau3, tau4
        )
        z_plus, _, z_plus_std, p_plus, _, _ = self.autoencoder.encoder(
            tau2, tau3, tau4, tau5
        )
    ...
    def load_model(self, name):
        ...
        checkpoint = torch.load(
            "RILI_co-adaptation/models/rili/{}.pt".format(name),
