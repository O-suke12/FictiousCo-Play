...
class SimpleEnv(AECEnv):
    ...
    def _execute_world_step(self):
        ...
        for agent in self.world.agents:
            ...
            if self.local_ratio is not None:
                ...
                if agent == self.fixed_agent:
                    reward += self.scenario.fixed_reward(self.world, self.fixed_agent,self.fixed_landmark)
            else:
                reward = agent_reward
    ...
    def draw(self):
        ...
        for e, entity in enumerate(self.world.entities):
            ...
            x += self.width // 2
            y += self.height // 2
            if entity == self.fixed_agent:
                pygame.draw.circle(
                    self.screen, (0, 255, 0), (x, y), entity.size * 350
                )
            elif entity is self.fixed_landmark:
                pygame.draw.circle(
                    self.screen, (0, 255, 0), (x, y), entity.size * 350
                )
            else:
