...
class Scenario(BaseScenario):
    def make_world(self, N=3):
        ...
        for i, agent in enumerate(world.agents):
            if i==0:
                agent.name = f"fixed_agent"
            else:
                agent.name = f"flex_agent"
            agent.collide = True
            agent.silent = True
            agent.size = 0.15
        # add landmarks
        ...
        for i, landmark in enumerate(world.landmarks):
            if i==0:
                landmark.name = f"fixed_landmark"
            else:
    ...
    def fixed_reward(self, world, fixed_agent, fixed_landmark):
        rew=0
        fixed_agent = fixed_agent
        fixed_landmark = fixed_landmark
        dist = np.sqrt(np.sum(np.square(fixed_agent.state.p_pos - fixed_landmark.state.p_pos)))
        rew -= dist
        return rew
