...
def main(cfg: DictConfig):
    ...
    env = resize_v1(env, cfg.frame_size[0], cfg.frame_size[1])
    env = frame_stack_v1(env, stack_size=cfg.stack_size)
    ...
    rb_obs = torch.zeros(
        (cfg.max_cycles, num_agents, cfg.stack_size, *cfg.frame_size)
    ).to(device)
    ...
    for episode in range(cfg.total_episodes):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(end_step)):
                delta = (
                    rb_rewards[t]
                    + cfg.gamma * rb_values[t + 1] * rb_terms[t + 1]
                    - rb_values[t]
                )
                rb_advantages[t] = delta + cfg.gamma * cfg.gamma * rb_advantages[t + 1]
        ...
        for repeat in range(3):
            ...
            for start in range(0, len(b_obs), cfg.batch_size):
