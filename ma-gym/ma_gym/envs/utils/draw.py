...
def fill_cell(image, pos, cell_size=None, fill="black", margin=0):
    ...
    x, y, x_dash, y_dash = (
        row + margin_x,
        col + margin_y,
        row + cell_size_x - margin_x,
        col + cell_size_y - margin_y,
    )
...
def write_agent_type(image, text, pos, cell_size=None, fill="black", margin=0):
    assert cell_size is not None and 0 <= margin <= 1
...
def draw_cell_outline(image, pos, cell_size=50, fill="black"):
    ...
    ImageDraw.Draw(image).rectangle(
        [(row, col), (row + cell_size_x, col + cell_size_y)], outline=fill, width=3
    )
...
def draw_border(image, border_width=1, fill="black"):
    ...
    new_im = Image.new(
        "RGB", size=(width + 2 * border_width, height + 2 * border_width), color=fill
    )
...
def draw_score_board(image, score, board_height=30):
    ...
    new_im = Image.new(
        "RGB", size=(im_width, im_height + board_height), color="#e1e4e8"
    )
