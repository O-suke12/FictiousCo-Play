...
class Lumberjacks(gym.Env):
    ...
    def __init__(
        self,
        grid_shape: Coordinates = (5, 5),
        n_agents: int = 2,
        n_trees: int = 12,
        agent_view: Tuple[int, int] = (1, 1),
        full_observable: bool = False,
        step_cost: float = -1,
        tree_cutdown_reward: float = 10,
        max_steps: int = 100,
    ):
        assert 0 < n_agents
        assert n_agents + n_trees <= np.prod(grid_shape)
        assert (
            1 <= agent_view[0] <= grid_shape[0] and 1 <= agent_view[1] <= grid_shape[1]
        )
        ...
        self.another_agent = None
        self.flex_agent = None
        ...
        obs_high = np.array([1.0] * self._obs_len, dtype=np.float32)
        obs_low = np.array([0.0] * self._obs_len, dtype=np.float32)
        ...
        self.observation_space = MultiAgentObservationSpace(
            [spaces.Box(obs_low, obs_high)] * self.n_agents
        )
    ...
    def get_action_meanings(
        ...
        if agent_id is not None:
            ...
            return [
                k.upper()
                for k, v in sorted(ACTIONS_IDS.items(), key=lambda item: item[1])
            ]
        else:
            return [
                [
    ...
    def reset(self, another_agent_type=None) -> List[List[float]]:
        self._init_episode()
        self._step_count = 0
        self._total_episode_reward = np.zeros(self.n_agents)
        self._agent_dones = [False] * self.n_agents
        self.steps_beyond_done = 0
        self.another_agent_type = another_agent_type
    ...
    def _generate_init_pos(self) -> np.ndarray:
        ...
        init_pos = np.array(
            ...
            + [PRE_IDS["empty"]]
            * (np.prod(self._grid_shape) - self.n_agents - self._n_trees)
        )
    ...
    def render(self, mode="human"):
        ...
        for pos, agent_strength, tree_strength in self._view_generator(mask):
            ...
            if tree_strength != 0:
                fill_cell(
                    img, pos=tree_pos, cell_size=cell_size, fill=TREE_COLOR, margin=0.1
                )
                write_cell_text(
                    img,
                    text=str(tree_strength),
                    pos=tree_pos,
                    cell_size=cell_size,
                    fill="white",
                    margin=0.4,
            ...
            if agent_strength != 0:
                if self.flex_agent.pos == self.another_agent.pos:
                    draw_circle(
                        img,
                        pos=agent_pos,
                        cell_size=cell_size,
                        fill=ImageColor.getcolor("purple", mode="RGB"),
                        radius=0.30,
                    )
                elif pos == (
                    self.another_agent.pos[0] - 1,
                    self.another_agent.pos[1] - 1,
                ):
                    draw_circle(
                        img,
                        pos=agent_pos,
                        cell_size=cell_size,
                        fill=ImageColor.getcolor("red", mode="RGB"),
                        radius=0.30,
                    )
                else:
                    draw_circle(
        ...
        write_agent_type(
            img, self.another_agent_type, (0, 0), cell_size=CELL_SIZE, fill="black"
        )
    ...
    def _view_generator(
        self, mask: Tuple[slice, slice]
    ) -> Tuple[Coordinates, int, int]:
    ...
    def get_agent_obs(self) -> List[List[float]]:
        ...
        obs[0] = obs[0] + obs[1][1:3]
        obs[1] = obs[1] + obs[0][1:3]
    ...
    def step(self, agents_action: List[int]):
        ...
        # Cut down trees
        mask = (np.sum(self._agent_map, axis=2) >= self._tree_map) & (
            self._tree_map > 0
        )
        self._tree_map[mask] = 0

        penalty_mask = (np.sum(self._agent_map, axis=2) == 1) & (self._tree_map == 2)

        # Calculate rewards
        rewards += np.sum(mask * self._tree_cutdown_reward, axis=(0, 1))
        rewards += np.sum(penalty_mask * self.invalid_penalty, axis=(0, 1))
        self._total_episode_reward += rewards

        if (self._step_count >= self._max_steps) or (
            np.count_nonzero(self._tree_map) == 0
        ):
    ...
    def _next_pos(self, curr_pos: Coordinates, move: int) -> Coordinates:
        ...
        if move == ACTIONS_IDS["noop"]:
            ...
            raise ValueError(
                "Unknown action {}. Valid action are {}".format(
                    move, list(ACTIONS_IDS.values())
                )
            )
    ...
    def seed(self, n: Union[None, int] = None):
        ...
        if n is not None:
            self.np_random = np.random.RandomState(seed=n)
        else:
