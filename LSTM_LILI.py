...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, z_dim, hidden_dim):
        super(ActorCritic, self).__init__()
        input_dim = state_dim + z_dim + hidden_dim
        hidden_dim = 256
...
class Latent_Predictor(nn.Module):
    def __init__(self, z_dim, hidden_dim):
        ...
        self.lstm = nn.LSTM(z_dim, self.hidden_dim, batch_first=True)
        self.latent_layer = nn.Linear(self.hidden_dim, z_dim)

    def forward(self, z, hidden):
        output, hidden = self.lstm(z, hidden)
        next_z = self.latent_layer(output)
...
class LILI:
    def __init__(
        ...
        **kwargs,
        ...
        self.device = device
        self.policy = ActorCritic(state_dim, action_dim, z_dim, hidden_dim).to(device)
        ...
        self.policy_old = ActorCritic(state_dim, action_dim, z_dim, hidden_dim).to(
            device
        )
        ...
        self.latent_predictor = Latent_Predictor(z_dim, hidden_dim).to(self.device)
        self.hidden = None
        self.lp_loss = nn.MSELoss()
    ...
    def select_action(self, state, t):
        with torch.no_grad():
            if t > 1:
                ...
                self.hidden = None
            next_z, self.hidden = self.latent_predictor(z, self.hidden)

            cell_state = self.hidden[1][0].detach().numpy()
            z = z.cpu().detach().numpy()
            state_latent = np.concatenate((state, z[0], cell_state))
            state = torch.FloatTensor(state_latent).to(self.device)
            action, action_logprob, state_val = self.policy_old.act(state)

        self.buffer.tau.append(tau)
        self.buffer.predicted_latents.append(next_z)
    ...
    def update(self):
        ...
        old_hidden_cells = (
            torch.squeeze(torch.stack(self.buffer.predicted_latents, dim=0))
            .detach()
            .to(self.device)
        )
        ...
        for _ in range(self.K_epochs):
            ...
            latents = torch.stack(self.buffer.state_all)[
                :, -(self.z_dim + self.hidden_dim) : -self.hidden_dim
            ]
            predicted_latents = []
            for latent in latents:
                if latent.sum() == 0:
                    self.hidden = None
                predicted_latent, self.hidden = self.latent_predictor(
                    latent.unsqueeze(0), self.hidden
                )
                predicted_latents.append(predicted_latent.squeeze(0))

            mask = torch.ones_like(latents)
            mask[::200] = 0
            mask = mask.bool()
            latents = torch.masked_select(latents, mask).view(-1, 5)

            predicted_latents = torch.stack(predicted_latents)
            mask = torch.ones_like(predicted_latents)
            mask[199::200] = 0
            mask = mask.bool()
            predicted_latents = torch.masked_select(predicted_latents, mask).view(-1, 5)

            hidden_loss = self.lp_loss(predicted_latents, latents)
            # self.latent_predictor(latents[0], self.hidden)
            # predicted_latents = self.latent_predictor[]
            # hidden_loss = self.lp_loss(predicted_latents, latents)
                ...
                self.run.log(
                    {
                        ...
                        "hidden_loss": total_hidden_loss.item(),
                    }
                )
    ...
    def save(self, checkpoint_path):
        ...
        torch.save(
            self.latent_predictor.state_dict(), checkpoint_path.replace("lili", "lp")
        )
    ...
    def load_ed_lp(self, checkpoint_path):
        if os.path.exists(checkpoint_path):
            ...
            self.latent_predictor.load_state_dict(
                torch.load(
                    checkpoint_path.replace("lili", "lp"),
                    map_location=lambda storage, loc: storage,
                )
            )
